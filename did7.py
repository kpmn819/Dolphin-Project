...
def show_rules():
    ...
    font = pygame.font.SysFont('FreeSans', 40, True, False)
    greeting = 'Hello Welcome to ID the Dolphin'
    greet_text = font.render(greeting, True, (50, 0, 0))
    display.blit(greet_text, (100, 100))
    pygame.display.flip()
    sleep(2)
    font = pygame.font.SysFont('FreeSans', 25, True, False)
    info = 'We can identify individuals by their dorsal fin shape'
    info_text = font.render(info, True, (50, 0, 0))
    display.blit(info_text, (150, 200))
    pygame.display.flip()
    sleep(2)
    inst = 'See if you can match one on the top row with the bottom picture'
    inst_text = font.render(inst, True, (255, 255, 255))
    display.blit(inst_text, (150, 250))
    pygame.display.flip()
    sleep(5)
...
def send_to_screen(left_pic, rnums, caption):
    ...
    for items in rnums:
        ...
        # use the rnums list to index comp_pic list to get the pictures
        display.blit(your_pic[rnums[items]],(rightx, righty))
